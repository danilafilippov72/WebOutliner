...
export class AdddMethod {
    ...
    public static Methods = class Methods {
        public static InformationHandler = class InformationHandler {
            ...
            public static ExtensionLackListeners() {
                /* document.querySelector('[identificator="' + this.Identificator + '"]').addEventListener(
                    "Outline",
                    function (event: CustomEvent) { InformationHandler.Initialize(event.detail) }
                ); */

                Object.values(AdddMethod.Elements).forEach(childElement => {
                    childElement.ExtensionLackListeners();
                    childElement.Methods.InformationHandler.ExtensionLackListeners()
                });
            }
            //#endregion Hidden
        };
        ...
        public static async Create(): Promise<HTMLDivElement> {
            ...
            Object.values(AdddMethod.Elements).forEach(async childElement => {
                global.count += 1;
                (childElement as typeof AdddMethod).Identificator = global.count.toString();
    ...
    public static ExtensionLackListeners() {
        document.querySelector('[identificator="' + this.Identificator + '"]').addEventListener(
            "Outline",
            function (event: CustomEvent) { AdddMethod.Methods.InformationHandler.Initialize(event.detail) }
        );
    }
