...
export class MethodsNode {
    ...
    public static Methods = class Methods {
        ...
        public static async Create(count: number): Promise<[HTMLDivElement, number]> {
            var count = count;
            aaaaa.Identificator = count.toString();
            if (aaaaa.InstanceIdentificator == null) aaaaa.InstanceIdentificator = count.toString();
            //#region ExtensionInterface
            let object = document.createElement("div");

            object.setAttribute("Identificator", count.toString());
            object.setAttribute("InstanceIdentificator", aaaaa.InstanceIdentificator);

            object = aaaaa.Form.Image.ApplyImage(object);
            
            if (aaaaa.Form.Image.Text != null) object.appendChild(document.createTextNode(aaaaa.Form.Image.Text));
            
            const array = Object.values(aaaaa.Elements);
            for (let index = 0; index < array.length; index++) {
                const childElement = array[index];
                const t = await (childElement as aaaaa).Methods.Create(count + 1);
                const child = t[0];
                const countAdd = t[1];
