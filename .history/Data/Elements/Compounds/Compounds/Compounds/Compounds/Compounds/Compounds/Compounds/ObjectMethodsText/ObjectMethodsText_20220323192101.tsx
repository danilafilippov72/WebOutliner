export class ObjectMethodsText {
    ...
    public static Methods = class Methods {
        ...
        public static async Create(count: number): Promise<[HTMLDivElement, number]> {
            var count = count;
            aaa.Identificator = count.toString();
            if (aaa.InstanceIdentificator == null) aaa.InstanceIdentificator = count.toString();
            //#region ExtensionInterface
            let object = document.createElement("div");

            object.setAttribute("Identificator", count.toString());
            object.setAttribute("InstanceIdentificator", aaa.InstanceIdentificator);

            object = aaa.Form.Image.ApplyImage(object);
            
            if (aaa.Form.Image.Text != null) object.appendChild(document.createTextNode(aaa.Form.Image.Text));
            
            const array = Object.values(aaa.Elements);
            for (let index = 0; index < array.length; index++) {
                const childElement = array[index];
                const t = await (childElement as aaa).Methods.Create(count + 1);
                const child = t[0];
                const countAdd = t[1];
